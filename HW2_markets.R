...
fileloc <- "C:\\Users\\ivano\\Desktop\\DSMMaster\\MLFin\\"
setwd(fileloc)
rm(fileloc)
...
saveRDS(data.env,file="./data/2/WorldMarkts99_20.RDS")
...
for(i in seq_along(markets)) {
  sym <- markets[i]
  returns <- merge(returns, periodReturn(Ad(get(sym,envir=data.env)),period=per,type = "log"))
