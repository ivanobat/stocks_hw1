...
ratio = 1-lambda
n = 1
mex_ema = sqrt(abs(EMA(mex_market, n=n, ratio=ratio)))
mex_sd = roll_sd(mex_ema, width=30)
mex_sma = SMA(mex_market, n=n, ratio=ratio) 
colnames(mex_sd) = c('SD')
colnames(mex_sma) = c('SMA')

merged = merge(mex_sma, mex_ema, mex_sd, all=FALSE)
```
We merge together the extracted standard deviation from the Exponential Moving Average and the rolling standard deviation with a 30-day window to make them comparable.

The graph shows that the SD from EMA oscillates a lot more given that it gives more importance to recent returns so it becomes more reactive.

Also as a baseline we include a simple moving average calculation

```{r}
mex_ema_plot = dygraph(data=merged, main='SD (from EMA) vs SD') %>%
  dyRangeSelector() %>%
  dyAxis('x', label = "Date") %>%
